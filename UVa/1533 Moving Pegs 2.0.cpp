...
const int ansn[15] = {9, 9, 9, 9, 10, 9, 9, 10, 10, 9, 9, 9, 9, 9, 9};
const int ans[15][25] = {
...
int main() {
    ...
    while (T--) {
        scanf("%d", &n);
        if (n < 1 || n > 15) printf("IMPOSSIBLE\n");
        else {
            printf("%d\n%d %d", ansn[n - 1], ans[n - 1][0], ans[n - 1][1]);
            for (int i = 1; i < ansn[n - 1]; i ++)
                printf(" %d %d", ans[n - 1][i * 2], ans[n - 1][i * 2 + 1]);
            printf("\n");
        }
    }
    return 0;
